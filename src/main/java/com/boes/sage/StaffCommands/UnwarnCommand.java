...
public class UnwarnCommand extends BaseCommand {
    ...
    public void onCommand(Player player, String targetName, @Optional Long timestamp) {
        ...
        if (timestamp != null) {
            if (plugin.getPunishmentManager().removeHistoryByTimestamp(target.getUniqueId(), timestamp)) {
                player.sendMessage("§aRemoved warning from " + target.getName());

                for (Player p : Bukkit.getOnlinePlayers()) {
                    if (p.hasPermission("sage.staff")) {
                        p.sendMessage("§e" + player.getName() + " §7removed a warning from §e" + target.getName());
                    }
                }
            } else {
                player.sendMessage("§cWarning with that timestamp not found!");
            }
        } else {
            ...
            for (int i = 0; i < warnings.size(); i++) {
                ...
                player.sendMessage("§7[§f" + (i + 1) + "§7] §fReason: §e" + warning.getReason() +
                        " §7| §fDate: §e" + date + " §7| §fTimestamp: §e" + warning.getTimestamp());
            }

            player.sendMessage("§7Use: /unwarn " + target.getName() + " <timestamp>");
        }
    }
}